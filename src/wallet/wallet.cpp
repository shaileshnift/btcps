...
void CWallet::AvailableCoinsForStaking(std::vector<COutput>& vCoins) const
{
    ...
    const bool includeColdStaking = false;
    vCoins.clear();
...
int CWalletTx::GetBlocksToMaturity() const
{
    ...
    //BTCSTODO Some wallets are still not abandoning coinstakes correctly
...
void CWallet::StakeBTCSs(bool fStake, CConnman* connman, ChainstateManager* chainman, CTxMemPool* mempool)
{
    ::StakeBTCSs(fStake, this, connman, chainman, mempool, stakeThread);
}

void CWallet::StartStake(CConnman* connman, ChainstateManager* chainman, CTxMemPool* mempool)
{
    m_enabled_staking = true;
    StakeBTCSs(true, connman, chainman, mempool);
}
